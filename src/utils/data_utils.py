...
class SpatialReasoningDataset(Dataset):
    """Generates synthetic or loads real spatial reasoning samples."""

    def __init__(self, synthetic=True, max_seq_len=256, num_samples=1000,
                 vocab_size=50257, data_path=None, visual_dim=512):
    ...
    def _tokenize(self, text):
        ...
        ids = [_deterministic_hash(w, self.vocab_size) for w in words[:self.max_seq_len]]
    ...
    def _load_real_sample(self, idx):
        ...
        input_ids = self._tokenize(
            f"{sample['context']} {sample['question']} {sample['answer']}"
        )
        labels = input_ids.clone()

        schema_idx = TASK_TYPE_MAP.get(
            sample.get("task_type", "unknown"), TASK_TYPE_MAP["unknown"]
        )
        schema_types = torch.full((self.max_seq_len,), schema_idx, dtype=torch.long)

        velocity = self._pad_2d(
            sample.get("velocity", np.random.randn(self.max_seq_len, 2) * 0.05),
            self.max_seq_len, 2,
        )
        coordinates = self._pad_2d(
            sample.get("coordinates", torch.cumsum(velocity, dim=0).tolist()),
            self.max_seq_len, 2,
        )
        temporal_distances = self._pad_1d(
            sample.get("temporal_distances", [1.0] * self.max_seq_len),
            self.max_seq_len, 1.0,
        )
        spatial_distances = self._pad_1d(
            sample.get("spatial_distances", torch.norm(coordinates, dim=-1).tolist()),
            self.max_seq_len, 0.0,
        )
        ...
        visual_features = self._pad_2d(
            sample.get("image_vector", np.zeros((1, self.visual_dim))),
            1, self.visual_dim,
        ).squeeze(0)
